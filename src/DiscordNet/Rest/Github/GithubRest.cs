...
namespace DiscordNet.Github
{
    public class GithubRest
    {
        ...
        {
            var result = await Task.WhenAll(numbers.Select(x => SendJsonRequestAsync(HttpMethod.Get, "/repos/Discord-Net-Labs/Discord.Net-Labs/issues/", x)));
        ...
        public async Task<PullRequest> GetPullRequestAsync(string number)
        {
            try
            {
                var result = await SendJsonRequestAsync(HttpMethod.Get, "/repos/Discord-Net-Labs/Discord.Net-Labs/pulls/", number);
        ...
        {
            var extra = $"?q=repo:Discord-Net-Labs/Discord.Net-Labs+language:cs+in:file{(filename == null ? "" : $"+filename:{filename}")}+{search.Replace(' ', '+')}&per_page=100";
            ...
            if (totalCount > 100)
            {
                ...
                for (int i = 2; i <= pages + 1; i++)
                {
                    extra = $"?q=repo:Discord-Net-Labs/Discord.Net-Labs+language:cs+in:file{(filename == null ? "" : $"+filename:{filename}")}+{search.Replace(' ', '+')}&per_page=100&page={i}";
